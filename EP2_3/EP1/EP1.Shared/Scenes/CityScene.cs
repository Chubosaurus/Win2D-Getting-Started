...
namespace Chubosaurus
{
    ...
    /// <summary>
    /// A Sample Parallax Scrolling scene of a City.
    /// </summary>
    public class CityScene : GenericLayeredScene
    {
        public CityScene(CanvasControl canvas = null) : base("City Scene")
        {            
            if (canvas != null)
            {
        ...
        protected override void SetupScene(CanvasControl canvas = null)
        {
            ...
            _title_label = new CGL_Label("City", Colors.White, (uint)(this._width * 0.60f), 100);
            _title_label.Y = 20;
            ...
            for (int i = 1; i < 9; i++)
            {
                GenericTiledBackground gi = new GenericTiledBackground(canvas, string.Format("bg_city_0{0}", i));
                gi.Velocity = i;
                gi.ZIndex = i;
                this.AddObject(gi);
            }
            ...
            ((GenericTiledBackground)this.objects[3]).Velocity = 5;
            ((GenericTiledBackground)this.objects[4]).Velocity = 10;
            ((GenericTiledBackground)this.objects[5]).Velocity = 20;
            ((GenericTiledBackground)this.objects[6]).Velocity = 130;
            ((GenericTiledBackground)this.objects[7]).Velocity = 130;         // ground            
